...
last_modified: 2025-11-19T11:20:06-06:00
...
The `operator` rule controls access to the [Operator API][api_operator] such as
...
operator {
  policy       = "read"
  capabilities = ["snapshot-save", "license-read", "keyring-rotate"]
}
```
...
- `deny` forbids any Operator API operation. Deny takes precedence over all
  other capabilities associated with a token.
- `keyring-rotate` allows rotating the root keyring.
- `keyring-delete` allows deleting keys from the root keyring.
- `keyring-read` allows reading key metadata from the root keyring.
- `license-read` allows reading the license of a Nomad Enterprise server.
- `snapshot-save` allows saving Raft snapshots.

The coarse-grained policy permissions implicitly grant the following
fine-grained capabilities.

| Policy  | Capabilities                                                                             |
|---------|------------------------------------------------------------------------------------------|
| `deny`  | `deny`                                                                                   |
| `read`  | `keyring-read`, `license-read`                                                           |
| `write` | `keyring-read`, `keyring-rotate`, `keyring-delete`,<br />`license-read`, `snapshot-save` |
...
The `sentinel` rule controls access to the [Sentinel API][api_sentinel] such
raft or scheduler configuration. The sentinel rule is optional, but you can
specify only one sentinel rule per ACL Policy.
...
- `deny` forbids any Sentinel API operation. Deny takes precedence over all
  other capabilities associated with a token.
- `sentinel-read` allows reading or listing Sentinel policies.
- `sentinel-submit` allows submitting new Sentinel policies or updating policies.
- `sentinel-delete` allows deleting Sentinel policies.
...
| `deny`  | `deny`                                                |
| `read`  | `sentinel-read`                                       |
| `write` | `sentinel-read`, `sentinel-delete`, `sentinel-submit` |
...
[api_sentinel]: /nomad/api-docs/sentinel/
