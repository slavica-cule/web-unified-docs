...
created_at: 2025-10-15T15:59:29+00:00
last_modified: 2026-02-17T22:25:54.000Z
...
1. We always recommend backing up your data tier before conducting maintenance or upgrade operations. The backup method will depend on your existing installation.
   ...
   -> **Note**: Throughout this guide, we use `terraform-enterprise` as the container name, which is the default for Replicated installations. If your container has a different name, adjust the commands accordingly. You can find your container name with: `sudo docker ps | grep terraform-enterprise`

   Back up the environment variables:
   ```shell-session
   $ sudo docker exec terraform-enterprise env | sudo tee env.txt > /dev/null
   ```
...
1. Copy your existing Replicated TLS certificates to the staging location:
   ...
   Retrieve the certificate paths from the container:
   ```shell-session
   $ sudo docker exec terraform-enterprise tfectl app config --unredacted | jq '{cert: .tls.cert_file, key: .tls.key_file, bundle: .tls.ca_bundle_file}'
   ```
   
   The output will show the certificate paths in the container. Depending on your setup, the file paths may differ from the following example:
   ```json
   {
   ...
   Use the paths from the output above to copy your certificates from the container to the host:
   ```shell-session
   $ sudo docker cp terraform-enterprise:/etc/ssl/private/terraform-enterprise/cert.pem /etc/terraform-enterprise/certs/
   $ sudo docker cp terraform-enterprise:/etc/ssl/private/terraform-enterprise/key.pem /etc/terraform-enterprise/certs/
   $ sudo docker cp terraform-enterprise:/etc/ssl/private/terraform-enterprise/bundle.pem /etc/terraform-enterprise/certs/
   ```
...
### Prepare HashiCorp license file

Contact your HashiCorp account manager to acquire a Flexible Deployment Options (FDO) license. This license is different from your existing Replicated license (.rli file). Store the license in `/etc/terraform-enterprise/license.hclic`.
...
~> **Important**: Back up your Terraform Enterprise data tier before proceeding. **If your organization has established backup procedures, you must follow those instead of the commands below.**

Refer to the [backup data tier guide](#back-up-data-tier) under the prerequisites section for recommended best practices and patterns.

The following commands provide a basic archive approach only and are not recommended for production use. This method does not follow backup best practices and should only be used if you have no other backup procedures in place. You proceed at your own risk. Ensure you validate the backup is complete and stored in a secure, redundant location before proceeding with migration.
...
1. We will refer to this path as the `${DISK_PATH}` going forward. Next, archive your mounted disk data:
   ...
   ```shell-session
   $ sudo tar -zcvf data.tar.gz -C ${DISK_PATH} aux postgres
   ```
...
1. Generate the Docker Compose configuration and save it to `/etc/terraform-enterprise/docker-compose.yml` using the following command:
   ```shell-session
   $ sudo docker exec terraform-enterprise tfectl app config --format docker | sudo tee /etc/terraform-enterprise/docker-compose.yml > /dev/null
   ```
...
1. Review the configuration file from the previous step. Update any values as needed before moving on to the next step. Pay special attention to placeholder values enclosed in `<>`, such as `image` and `TFE_LICENSE`, and replace them with your actual values.
   ...
   ```shell-session
   $ cat /etc/terraform-enterprise/docker-compose.yml
   ```

   Reference your backed up environment variables for configuration values:
   ```shell-session
   $ cat env.txt
   ```

   Edit the compose file (use your preferred editor):
   ```shell-session
   $ sudo nano /etc/terraform-enterprise/docker-compose.yml
   ```

   Validate the compose file syntax after making changes:
   ```shell-session
   $ sudo docker compose -f /etc/terraform-enterprise/docker-compose.yml config
   ```
...
1. Log in to the Terraform Enterprise container image registry.
   ...
   ```shell-session
   $ cat <PATH_TO_HASHICORP_LICENSE_FILE> | sudo docker login --username terraform images.releases.hashicorp.com --password-stdin
   ```
...
1. Pull the Terraform Enterprise image from the registry.  Refer to [Terraform Enterprise Releases](https://developer.hashicorp.com/terraform/enterprise/releases) for a list of versions and replace `<vYYYYMM-#>` with the version you wish to install; `latest` is not a valid tag.  
   ...
   ```shell-session
   $ sudo docker pull images.releases.hashicorp.com/hashicorp/terraform-enterprise:<vYYYYMM-#>
   ```

1. Create a new `systemd` service for Terraform Enterprise by creating a `/etc/systemd/system/terraform-enterprise.service` file with [the contents on the Docker Installation guide](/terraform/enterprise/deploy/docker). First, verify your Docker installation paths:

   ```shell-session
   $ which docker
   ```

   Use the following command to create the systemd service file. The example below assumes Docker is installed at `/usr/local/bin/docker`. Adjust the paths in the configuration if your Docker installation differs:
...
1. Stop Terraform Enterprise (Replicated). Ensure the application has fully stopped before proceeding.
   ...
   ~> **Reminder:** If you have not already backed up your data, follow the steps in [Back up your data tier](#step-1-backup-your-data-tier) before proceeding, as the migration occurs in the following steps.

1. Start (and enable on start up) the Docker Compose based Terraform Enterprise.

   ```shell-session
   $ sudo systemctl enable --now terraform-enterprise
   ```
...
1. Check the status of your deployment:
   ...
   To view logs, find your container name and then check the logs:
   ...
   ```shell-session
   $ curl https://jrep-eol.ptfe-dev.aws.ptfedev.com/_health_check
   ```

   Terraform Enterprise should now be running using Docker Compose. When confirmed, you can shut down and disable your Replicated services.

1. Shut down Replicated.

   ```shell-session
   $ sudo systemctl disable --now replicated replicated-ui replicated-operator
   ```
...
1. We recommend reusing your Replicated certificate to minimize the upgrade's effect on your other stack components.
   ...
   If you cannot access your certificate, key, or bundle file, you can retrieve them from the Replicated Terraform Enterprise container. Run the following command to find your container name:

   ```sh
   docker ps | grep terraform-enterprise
   ```
   
   The container name is typically `terraform-enterprise` for Replicated installations. Use this name from the output in the following commands.
   
   Retrieve the certificate paths:

   ```sh
   docker exec <CONTAINER_NAME> tfectl app config --unredacted | jq '{cert: .tls.cert_file, key: .tls.key_file, bundle: .tls.ca_bundle_file}'
   ```
   ...
   ```sh
   docker cp <CONTAINER_NAME>:/etc/ssl/private/terraform-enterprise/cert.pem <PATH_TO_CERTS_ON_HOST>/cert.pem
   docker cp <CONTAINER_NAME>:/etc/ssl/private/terraform-enterprise/key.pem <PATH_TO_CERTS_ON_HOST>/key.pem
   docker cp <CONTAINER_NAME>:/etc/ssl/private/terraform-enterprise/bundle.pem <PATH_TO_CERTS_ON_HOST>/bundle.pem
   ```
