...
created_at: 2025-07-03T17:17:22+00:00
last_modified: 2026-02-18T00:05:06.000Z
...
1. Create a new `systemd` service for Terraform Enterprise by creating a `/etc/systemd/system/terraform-enterprise.service` file with [the contents on the Docker Installation guide](/terraform/enterprise/deploy/docker). First, verify your Docker installation paths:
   ...
   Use the following command to create the systemd service file. The example below assumes Docker is installed at `/usr/bin/docker`. Adjust the paths in the configuration if your Docker installation differs:
