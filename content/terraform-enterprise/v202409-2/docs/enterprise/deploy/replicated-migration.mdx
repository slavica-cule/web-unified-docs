...
1. We always recommend backing up your data tier before conducting maintenance or upgrade operations. The backup method will depend on your existing installation.
   ...
   ~> **Note**: If you customized your Terraform Enterprise container name then replace `terraform-enterprise` with your actual container name in the commands on this page.
...
Your Terraform Enterprise instance requires TLS/SSL certificates to operate securely. Terraform Enterprise supports two modes for TLS certificates:

1. **Application-managed TLS**: Terraform Enterprise terminates TLS connections and serves the certificate. In this mode, you must provide the certificate and private key to Terraform Enterprise.
1. **External TLS termination**: A load balancer or reverse proxy terminates TLS connections before they reach Terraform Enterprise. Terraform Enterprise does not need access to the certificate in this mode.

Refer to [TLS certificates](/terraform/enterprise/deploy/configuration/network/certificate) for instructions on how to prepare your certificates for either mode.

#### Application-managed TLS

If you are using application-managed TLS, ensure you have the following files prepared:
- **TLS certificate file** (fullchain PEM format, including any intermediate certificates)
- **TLS private key file** (PEM format)

The installation procedure will reference these files. If your current Replicated installation is using application-managed TLS, you can export the certificate and key from the Replicated configuration.

#### External TLS termination

If you are using external TLS termination, ensure your load balancer or reverse proxy is properly configured to forward traffic to Terraform Enterprise on the appropriate port (typically port 443 for HTTPS).

For external TLS configurations, you do not need to provide certificates to Terraform Enterprise. However, you must configure Terraform Enterprise to operate in external TLS mode using the `TFE_TLS_ENFORCE=false` environment variable.

### Prepare HashiCorp license file

Ensure you have obtained a Flexible Deployment Options (FDO) license file from your HashiCorp account representative. This license file is required to run Terraform Enterprise on a non-Replicated runtime and should be in `.rli` format.
...
Verify that you have the required Docker and Docker Compose versions installed:
...
1. Generate the Docker Compose configuration and save it to `/etc/terraform-enterprise/docker-compose.yml` using the following command:
   ```shell-session
   $ sudo docker exec terraform-enterprise tfectl app config --format docker | \
     sudo tee/etc/terraform-enterprise/docker-compose.yml > /dev/null
   ```
...
1. Verify you have the Docker CLI available:
...
1. Create a new `systemd` service for Terraform Enterprise. Create the `/etc/systemd/system/terraform-enterprise.service` file with the following contents:
   ...
   ExecStart=/usr/bin/docker compose -p terraform-enterprise up -d
   ExecStop=/usr/bin/docker compose -p terraform-enterprise down
...
1. Stop Terraform Enterprise (Replicated). Ensure the application has fully stopped before proceeding.
   ...
   ```
1. ~> **Important**: If you have not already backed up your data tier, do so now before proceeding. Refer to the [backup data guide](#back-up-data-tier) under prerequisites for detailed instructions.
...
1. Check the status of your service with `sudo systemctl status terraform-enterprise`, or use `sudo docker ps` to find your container's name then run `sudo docker logs [name]`. You can also run `curl https://[hostname]/_health_check` to check the health check endpoint. Terraform Enterprise should now be running using Docker Compose, so your Replicated services can be shut down and disabled.
